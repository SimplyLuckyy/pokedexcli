...
func (c *Client) ListPokeINFO(pokename string) (PokeAPIPokeINFO, error) {
	url := baseURL + "/pokemon/" + pokename

	val, ok := c.cache.Get(url)
	if ok {
		pokeINFO := PokeAPIPokeINFO{}
		err := json.Unmarshal(val, &pokeINFO)
		if err != nil {return PokeAPIPokeINFO{}, err}
